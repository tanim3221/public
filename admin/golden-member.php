			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-md-12">
						...
						<h2 class="page-title">Golden Member</h2>
				
							<?php if($error){?><div class="errorWrap"><strong>ERROR</strong>:<?php echo htmlentities($error); ?> </div><?php } 
				else if($msg){?><div class="succWrap"><strong>SUCCESS</strong>:<?php echo htmlentities($msg); ?> </div><?php }?>
				<div class="btn-data-download">
				<a class="btn btn-primary" type="button" href="download-records-GM.php">Download GM List</a></div>
...
<?php $sql = "SELECT * from  lifemembers WHERE CategoryM IN ('GM') ORDER by MemId ";
$query = $dbh -> prepare($sql);
$query->execute();
...
{				?>	
				...
				{?>
				<a type="button" class="btn btn-warning" href="golden-member.php?comm=<?php echo htmlentities($result->id);?>" onclick="return confirm('Do you really want to add to committee?')">  Add to Committee </a> 
				<?php } else {?>

				<a type="button" class="btn btn-info" href="golden-member.php?noncomm=<?php echo htmlentities($result->id);?>" onclick="return confirm(' Do you really want to remove from committee?')">  Remove </a>
				...
				{?>
				<a type="button" class="btn btn-danger" href="golden-member.php?public=<?php echo htmlentities($result->id);?>" onclick="return confirm('Do you really want to avtive this member?')">  Inactive </a> 
				<?php } else {?>
            <div class="btn-group-vertical">
				<a type="button" class="btn btn-success" href="golden-member.php?hidden=<?php echo htmlentities($result->id);?>" onclick="return confirm('Do you really want to inavtive this member?')">  Active </a>
...
<td>
			...
			<td>  
				...
				{?>
				<a type="button" class="btn btn-warning" href="golden-member.php?participants=<?php echo htmlentities($result->id);?>" onclick="return confirm('Do you really want to add to participants List?')">  Add to  <br> Participants List </a> 
				<?php } else {?>

				<a type="button" class="btn btn-info" href="golden-member.php?nonparticipants=<?php echo htmlentities($result->id);?>" onclick="return confirm(' Do you really want to remove from participants List?')"> Removed from <br> Participants List </a>
